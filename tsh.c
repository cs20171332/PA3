...
void eval(char *cmdline) 
{
    ...
    if(check==0)
    {
        ...
        else if(eval_pid>0)
        {
            if(bg!=0)
            {
                addjob(jobs, eval_pid, BG, cmdline);
                printf("[%d] (%d) %s", pid2jid(eval_pid), eval_pid, cmdline);
            }
            else
            {
                addjob(jobs, eval_pid, FG, cmdline);
                waitfg(eval_pid);
            }
        }
    }
...
void waitfg(pid_t pid)
{
    struct job_t *fore_jobs = getjobpid(jobs, pid);
    if(fore_jobs!=NULL)
    {
        while(fore_jobs->pid==pid && fore_jobs->state==FG)
        {
            sleep(1);
        }
    }
